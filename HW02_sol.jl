...
# ╔═╡ 43432aac-8167-42b1-bf9f-3edfe9ffc288
extrema(img)
...
function to_equal_intensity_tuple(value::T) where T
    ...
    red_map(x) = (1 + sin(x * T(2π)  + 2 * T(2π) /3))/3 # TODO
    green_map(x) = (1 + sin(x * T(2π))) / 3 # TODO
    blue_map(x) = (1 + sin(x * T(2π) + T(2π) / 3)) /3 # TODO

    return (red_map(value), green_map(value), blue_map(value)) # TODO, return the correct tuple!
end

# ╔═╡ 77de2c3a-bd3b-48b4-84fa-b69b82bdd459
sum(to_equal_intensity_tuple(1.0))

# ╔═╡ f94c6f03-d032-4670-b657-ac76f4f0df08
sum(to_equal_intensity_tuple(0.2323))

# ╔═╡ 8aa300f5-c597-4622-9a7b-e7e15191a329
sum(to_equal_intensity_tuple(0.01))

# ╔═╡ 332792d7-40c0-478d-864a-1c6e8fc64333
typeof(2π)

# ╔═╡ ebffd14e-97d9-4e6f-8a84-c56991f78302
Float32(2π)
...
# ╔═╡ 35bf965a-b2b9-432a-8ca0-9d50ff356470
@show create_equal_intensity_colormap()

# ╔═╡ 78cd90b6-6a9f-49ea-9cb6-5959f420338e
RGB(1, 0.1, 1)
...
function to_negative_positive_tuple(value::T) where T
    ...
    # fix those three functions such that the property is achieved
    # maybe think about periodic functions ;)
    red_map(x) = x < 0.5 ? 2 * x : Tr(1)
    green_map(x) = x < 0.5 ? 2 * x : 1 - (x-Tr(0.5)) * 2
    blue_map(x) = x < 0.5 ? Tr(1) : 1 - (x-Tr(0.5)) * 2

    return (red_map(value), green_map(value), blue_map(value)) # TODO, return the correct tuple!
end

# ╔═╡ 9fa2363f-1ea7-4a1f-a701-fb048249a1ca
begin
    r2,g2,b2 = unzip(to_negative_positive_tuple.(x))
    plot(x, r2, color=:red, label="red")
    plot!(x, g2, color=:green, label="green")
    plot!(x, b2, color=:blue, label="blue")
end
...
begin
    function create_negative_positive_intensity_colormap()
        values = range(0, 1, length=128)
        return map(x -> RGB(x...), to_negative_positive_tuple.(values))
    end
    my_negative_positive_colormap = create_negative_positive_intensity_colormap()
...
# ╔═╡ fd05f4a1-d9cb-4f78-b68c-51757292ac33
heatmap(img, c=:bwr)

# ╔═╡ 711ddbd8-4f8c-4988-b08f-fbd44d37e915
heatmap(img, c=:julia)
...
function simulate_pixel(n_photons; read_σ=5, offset=0.5, gain=0.1)
    return (pois_rand(n_photons) + randn() * read_σ) * gain + offset
end
...
# expects the integer of the events as input (in our case photons)
pois_rand()
...
# ╔═╡ 23a0b06b-63bc-4dd0-91fa-d0e341b944b3
b1 = (0.0123 ≈ simulate_pixel(0, read_σ=0, offset=0.0123, gain=1))

# ╔═╡ 02b3bafb-fc03-42ff-a281-47216a04f6ce
# check offset
PlutoTest.@test b1

# ╔═╡ d3e8cab1-84ca-4dc3-81e9-74df4818e013
b3 = ≈(123, mean(simulate_pixel.(ones((1000,)) * 123, read_σ=0, offset=0.0123, gain=1)), rtol=0.1)

# ╔═╡ 1d3e3c6f-7920-4b2c-a432-105fa1f131f7
# check mean poisson
PlutoTest.@test b3

# ╔═╡ 578cf779-7238-4366-8660-76b56ca1241e
b4 = ≈(123, var(simulate_pixel.(ones((1000,)) * 123, read_σ=0, offset=0.0123, gain=1)), rtol=0.1)

# ╔═╡ b01359d9-c1b2-455f-897c-8ab6bed4f20f
# check variance poisson
PlutoTest.@test b4

# ╔═╡ 8784c09c-4ce3-4eee-b0e7-1dcf76a9dd7f
b5 = ≈(12.3,std(simulate_pixel.(zeros((1000,)) * 123, read_σ=12.3, offset=0, gain=1)),rtol=0.1)

# ╔═╡ a598763f-63f9-48c9-9e5b-ba6256775221
# check readnoise
PlutoTest.@test b5

# ╔═╡ 7b634dea-9c35-4cba-a575-cdf08c4fb8a9
b6 = ≈(42 * 12.3 , std(simulate_pixel.(zeros((1000,)) * 123, read_σ=12.3, offset=0, gain=42)), rtol=0.1)
...
begin
    ...
    gauss = Float32.(exp.(-0.03 .* (x_cords.^2 .+ x_cords'.^2)))
    gauss ./= sum(gauss)
...
# ╔═╡ 867e56f2-a4eb-4d7f-9b60-4e622dfc6c0c
function mean3(arr)
    m = zeros(eltype(arr), size(arr,1), size(arr, 2))
...
$$\frac{\hat \sigma^2}{\hat \mu} = \text{gain}$$

Dividing $\hat \mu$ by $\text{gain}$ returns the measured photon number.

Reshuffle this equation to:

$${\hat \sigma^2} = \text{gain} {\hat \mu}$$
...
md"
`offset`=$(@bind offset Slider(0.0:1:100, show_value=true))

`read_σ`=$(@bind read_σ Slider(0:1:20, show_value=true))
...
begin
    imgs = repeat(img_blurry, outer=(1, 1, 30));
...
# ╔═╡ f79c842c-8fcb-41cb-9d7a-3f20f3da84fa
@time mean3(images);

# ╔═╡ b8c49fac-685a-4e54-9f7c-11d1876a8438
@time mean(images, dims=3);

# ╔═╡ 70970178-dd1c-49be-a7df-e374365c5e80
mean3(images) ≈ μ_arr
...
scatter(μ, variance, markersize=0.3,xlabel="mean", ylabel="variance")
