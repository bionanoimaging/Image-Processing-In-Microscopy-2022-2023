...
function add_gauss_noise(img, σ=one(eltype(img)))
	# randn should get the size of the image
	img2 = img .+ σ * randn(size(img))
	return img2
end

# ╔═╡ 653b16db-e129-4bf2-bd4a-63d1a9b3ffab
x = [1.0,1.0,2.0]

# ╔═╡ a16288a9-1a44-4dc0-bff5-f9722db83114
add_gauss_noise(x)

# ╔═╡ 7cfe8d7d-c7fe-487f-bc53-664b678c13ec
x

# ╔═╡ aeb7a6f4-24fc-4f94-9fe8-d8b8b05df41b


# ╔═╡ 4c5cc372-c421-4560-880d-eb865cd06347
...
# ╔═╡ 45894fe9-cb40-4b25-a2a5-289276df6531
size(img, 2)

# ╔═╡ 4ac728a8-59a2-4f32-8d2e-dddd8fb59419
bbb = [1.0,2.0,3.0]

# ╔═╡ e0fcb2d2-ebdf-47df-a073-ce1065258928
add_gauss_noise_fl!(bbb)

# ╔═╡ fa12b65e-cfe4-40a6-a136-cf26ee2b54a7
# original array is indeed modified
bbb
...
# ╔═╡ 28c68477-6aeb-4fd5-a9bd-ef03f9a4d26e
std(add_gauss_noise(ones((512, 512)), 0.3))
...
# ╔═╡ fb7933b8-794c-43f8-b3d7-f0e94617d558
# use broadcasting mechanism
ppp = pois_rand.(12345 .* ones((512, 512)))

# ╔═╡ 3b87124d-a122-4298-bebe-e9ad06a26055
mean(ppp)

# ╔═╡ 3ba635db-e664-4e8f-9ed4-81936a4a9648
var(ppp)
...
function add_poisson_noise!(img, scale_to=nothing)
	...
	# scale to represents a photon count
	else
		# can be for example 0.9
		max_of_img = maximum(img)


		# pois_rand needs a photon count and not digital values
		img ./= max_of_img
		img .*= scale_to
...
# ╔═╡ 0487df5c-dcab-4099-823a-176c23c20c62
add_poisson_noise!([1.0, 2.0, 1.0], 120)

# ╔═╡ 924682c5-b6c1-418d-8ddc-13a06a73eff1


# ╔═╡ 43bad00e-f88c-4a98-836b-bfc42cb53763
my_show(add_poisson_noise!(copy(img)))

# ╔═╡ 02706594-23af-40b6-9d30-3bf953f550cb
my_show(add_poisson_noise!(copy(img), 10))

# ╔═╡ 8c9f0425-99aa-48c6-b5a1-eafe5fc52f4d
...
# ╔═╡ 263f1f9d-af4a-4964-8af6-b6e3e5a0af3b
number = 10000000000000

# ╔═╡ 5fb03ad8-9b11-4a62-b389-2b18f9ba9314
(pois_rand(number) - number) / number
...
function add_hot_pixels!(img, probability=0.1; max_value=1)
	...
	# a more generic approach for for-loops
	# it works multidimensional arrays
...
# ╔═╡ 22c80518-6e73-4ecf-a642-4ce583a9dc89
function add_hot_pixels_vectorized!(img, probability=0.1; max_value=1)
	# vector
	#binary mask full of 1s and 0s
	mask = (rand(size(img)...) .< probability)
	img[mask] .= max_value
	
	return img
end

# ╔═╡ 02a920d6-430a-449f-9202-8423fde9a47b
@time add_hot_pixels!(copy(img));

# ╔═╡ 74cc049d-6ae8-4022-a2e8-46eb9fba2564
@time add_hot_pixels_vectorized!(copy(img));

# ╔═╡ 2070d7f6-5ca7-4e31-9c9f-88a7fd71374f
rand(2,3)

# ╔═╡ 1eae7438-fa19-4a59-bfe4-1e83a7ece81c
rand()

# ╔═╡ 22985061-9740-45e9-af56-e0787dadba7b
my_show(add_hot_pixels!(copy(img), 0.001))

# ╔═╡ b6c25124-3165-4c85-9b1c-da68bd68b59f
my_show(add_hot_pixels!(copy(img), 0.1; max_value=3))
...
begin
	...
	img_h = add_hot_pixels!(copy(img), 0)
end;
...
# ╔═╡ 7d2bb5b6-18cd-44ff-9853-3ec1f36cf0f9
sum(normal((100,100), sigma=12))

# ╔═╡ ded0b153-3ba6-4ad6-906b-87930e2b82d0
# check out the help
sum(normal((10,10), sigma=1))

# ╔═╡ 9fbf710a-3f00-4bd6-9d51-ed9661338772


# ╔═╡ 6acf7ba6-445d-4ac0-a83f-6916318c783c
function gaussian_noise_remove(arr; kernel_size=(3,3), σ=1)
	kernel = normal(kernel_size, sigma=σ)

	# normalize the sum of the kernel to 1
	# otherwise, you are reducing the intensity your image 
	
	kernel = kernel ./ sum(kernel)
	
	# imfilter is bascially a convolution
...
# ╔═╡ 76a56216-834e-4f8a-aa43-2ebb650dcd81
median([1,2,3,4,5,6])

# ╔═╡ 916b17d1-4d5e-4cad-9b6e-a774823583f8
yy = randn((4,4))

# ╔═╡ e22981f9-3128-4f05-a97b-c0bfd17b8455
yy[1:3, 2:3]

# ╔═╡ 27b12879-af98-4240-ba7e-fe8524f77ee6
clamp.(-1:13, 1, 10)

# ╔═╡ 147db608-d5e6-443f-992f-101072a5d3e7
begin
	i = 10
	j = 20
	kernel_size = (4,4)
	range_i = i - (kernel_size[1]) ÷ 2: i + (kernel_size[1] - 1)÷ 2
end

# ╔═╡ 7bc0f845-d5d4-4192-be1d-b97750e95761
function median_noise_remove!(img; kernel_size=(5,5))

	img_input = copy(img)
	for i = 1:size(img, 1)
		for j = 1:size(img, 2)
			
			range_i = i - kernel_size[1] ÷ 2: i + kernel_size[1] ÷ 2
			range_j = j - kernel_size[2] ÷ 2: j + kernel_size[2] ÷ 2

			range_i_c = clamp.(range_i, 1, size(img, 1))
			range_j_c = clamp.(range_j, 1, size(img, 2))

			box = img_input[range_i_c, range_j_c]
			img[i, j] = median(box)
		end
	end
...
# ╔═╡ 0aea25e9-f57e-401a-a8ca-8ac874734b31
begin
	div(5, 2)
...
# ╔═╡ ea598389-f010-4b81-90e9-3188df72fac6
÷
...
# ╔═╡ 46e221e3-98f3-4ffe-8c20-f65a23c43084
img_noisy_hot = add_hot_pixels!(copy(img), 0.3);

# ╔═╡ d05e790b-0476-4700-8b4b-e7cd6c43349a
my_show([img_noisy_hot;; median_noise_remove!(copy(img_noisy_hot), kernel_size=(7,7))])

# ╔═╡ 20aa7d48-489e-4804-9b1f-193e5f245133
