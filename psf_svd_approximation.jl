...
# ╔═╡ 3d5296a6-6cc3-11ed-1247-218ab77b06cc
using Colors, FourierTools, ImageShow, ImageIO, IndexFunArrays, PlutoUI, Noise

# ╔═╡ ade4c0db-fabe-44f8-b1f8-6440850d563a
using LinearAlgebra

# ╔═╡ ac223258-f613-4c86-a588-60e475f85ad0
begin
...
# ╔═╡ 8409c052-70bf-4d6f-a0fd-3885bf2a25e7
md"## Calculate PSF"

# ╔═╡ 91a85bbc-7575-4ba0-8ced-0aeef7a0839d
_psf = abs2.(ift(disc(Float32, (256, 256), 6)));

# ╔═╡ ede5ff1b-6e82-44f7-bdfb-c35b376245e2
psf = _psf ./= maximum(_psf);

# ╔═╡ 2a889abf-e53f-4cbe-aacb-5721601aacf1
simshow(psf, γ=0.3)

# ╔═╡ fb71f012-b8ff-4cf9-b6b4-93a92c67f4b2


# ╔═╡ 1fafc332-b7b6-4ba4-bbb4-2ce208883c35
md"## Calculate SVD"

# ╔═╡ 083c8dd7-8465-4bf0-8014-a0c9e4660ac4
F = svd(psf);

# ╔═╡ 923de2cb-ca00-4369-b889-d84d6e6e8782
U, S, Vt = F.U, F.S, F.Vt;

# ╔═╡ 68858d3c-6fa6-4019-a135-451effc8f511
simshow(U * Diagonal(S) * Vt, γ=0.3, set_zero=true)

# ╔═╡ 03eac987-dd8f-4e0c-bb84-adf578a78c78
simshow(U[:, 1:1] * Diagonal(S[1:1]) * Vt[1:1, :], γ=0.3)

# ╔═╡ 0f1d6325-15e2-4016-8eb0-9216bd1f3ff6
function assemble(U, S, Vt, i_lim)
	out = zeros(eltype(U), size(U))

	for i = 1:i_lim
		out .+= U[:, i:i] * Diagonal(S[i:i]) * Vt[i:i, :]
	end
	return out
end

# ╔═╡ 7592be74-eb07-4519-8c66-1cc3fadbe7c6
@bind is Slider(1:size(psf, 1), show_value=true)

# ╔═╡ eff2a813-cf8c-4ef7-8e9f-a9a3e140647e
md"## Assemble with $is principal components"

# ╔═╡ 5bde77f2-baec-4b7d-a86c-27db0cf27600
psf_approx = assemble(U, S, Vt, is);

# ╔═╡ 206a6e0b-c02d-43de-830f-a2544d9fd6eb
[simshow(psf, γ=0.3) simshow(psf_approx, γ=0.3, set_zero=true) simshow(psf .- psf_approx, set_zero=true, set_one=false, γ=0.3)]

# ╔═╡ 0533f450-1efb-494a-a4e8-d683ee3eaf97
md"# Relative Error"

# ╔═╡ 24e3d920-9ae8-44fa-b144-fca6c793200b
sum(psf .- psf_approx) ./ sum(psf) 
...
julia_version = "1.8.2"
manifest_format = "2.0"
project_hash = "1e888bfa539a76b347d5e793a182727aae123896"
